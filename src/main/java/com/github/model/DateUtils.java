...
public class DateUtils {
    ...
    public static TreeMap<Long, Long> twoWeekIntervalCalc = new TreeMap<Long, Long>();
    static 
    {
        Calendar c = Calendar.getInstance();
        ...
        while (year < c.get(Calendar.YEAR))
        {
            ...
            twoWeekIntervalCalc.put(min, max);            
        }
    }
    public static Pair<Long, Long> getCurrentTwoWeekPeriod()
    {
        long key = Calendar.getInstance().getTimeInMillis();
        Map.Entry<Long, Long> rangedDate = twoWeekIntervalCalc.floorEntry(key);
        return new ImmutablePair<Long, Long>(rangedDate.getKey(), rangedDate.getValue());
    }
}
